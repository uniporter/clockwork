...
class PureToken<F extends Formattable> {
    ...
    /// Returns a [PureToken] that augments this with fallbacks. That is, when this acting on an [Formattable] returns `null`,
    /// we will act tokens in [fallbacks] on the [Formattable] one by one until a non-null value is found with the value then returned.
...
class StatefulToken<F extends Formattable> {
    final String? Function(F f, Calendar calendar, Locale locale) _formatter;
    final String identifier;
    const StatefulToken(this._formatter, [this.identifier = 'StatefulToken']);

    String? call(F f, Calendar c, Locale l) => _formatter(f, c, l);

    /// Returns a new token that's a clone of this.
    StatefulToken<F> clone([String identifier = 'StatefulToken']) => StatefulToken((f, c, l) => this(f, c, l), identifier);

    /// Returns a new token that formats the numeral returned by this at as long as it takes
    /// to display the numeral fully, then pads it so the resulting length is at least [len].
    ///
    /// If [unpadded] is set, then we assume that this doesn't have any padding; the function will run faster as a result.
    StatefulToken<F> withMinLength(int len, [bool unpadded = false, String identifier = 'StatefulToken']) => StatefulToken((f, c, l) {
    ...
    /// Returns a [StatefulToken] that augments this with fallbacks. That is, when this acting on an [Formattable] returns `null`,
    /// we will act tokens in [fallbacks] on the [Formattable] one by one until a non-null value is found with the value then returned.
...
extension PureTokenIterableExtension<F extends Formattable> on Iterable<PureToken<F>> {
    /// Returns a token that's a concatenation of tokens in this.
    PureToken<F> concat() => PureToken((f) => map((token) => token(f)).join());
}

extension StatefulTokenIterableExtension<F extends Formattable> on Iterable<StatefulToken<F>> {
    /// Returns a token that's a concatenation of tokens in this.
